...
let gen_tests vs =
  ...
  (List.init vs ~f:(fun root -> (Random_tree.with_root ~root:root, (Printf.sprintf "Random_tree_with_root_%d" root))))

let test ~fname ~graph ~iters =
  let sts, rts = Kruskal.gen_all_sts ~graph in
  ...
  in
  let tests = gen_tests graph.v in
  let rdist = Hashtbl.fold rts ~init:"" ~f:(fun ~key ~data acc -> Printf.sprintf "%s\t\t%s: %f\n" acc key (data *. Float.of_int iters)) in
  (Printf.sprintf "Expected distribution:\n%s\n" rdist) :: List.map tests ~f:test
